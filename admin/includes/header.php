...
<<<<<<< HEAD
            ...
            <a href="/admin/moderators/list.php" class="nav-link <?= strpos($_SERVER['PHP_SELF'], '/moderators/') !== false ? 'active' : '' ?>">
                <i class="fas fa-user-edit me-2"></i> Модераторы
                <?php
                try {
                    $pending_requests = $pdo->query("SELECT COUNT(*) FROM moderator_requests WHERE status = 'pending'")->fetchColumn();
                    $pending_withdrawals = $pdo->query("SELECT COUNT(*) FROM withdrawal_requests WHERE status = 'pending'")->fetchColumn();
                    $total_pending = $pending_requests + $pending_withdrawals;
                    if ($total_pending > 0):
                ?>
...
<<<<<<< HEAD
            ...
            <a href="/admin/album/constructor.php" class="nav-link <?= strpos($_SERVER['PHP_SELF'], '/album/') !== false ? 'active' : '' ?>">
                <i class="fas fa-book-open me-2"></i> Конструктор альбома
            </a>
            <?php endif; ?>
