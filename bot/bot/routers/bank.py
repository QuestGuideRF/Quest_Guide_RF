...
<<<<<<< HEAD
        ...
        await callback.message.edit_text(
            text,
            reply_markup=BankKeyboards.topup_for_route(needed, route_id, user.language),
            parse_mode="HTML"
        )
        await callback.answer()
=======
...
<<<<<<< HEAD
    if getattr(user, "referred_by_id", None):
        try:
            ...
            await ref_service.process_purchase(user, route_id, Decimal(str(route.price)))
        except Exception as e:
            import logging
    ...
    if route.creator_id:
        try:
            ...
            commission_result = await session.execute(
                text("SELECT value FROM platform_settings WHERE `key` = 'commission_percent'")
            )
            commission_percent = Decimal(commission_result.scalar() or "10")
            total_amount = Decimal(str(route.price))
            platform_amount = (total_amount * commission_percent / Decimal("100")).quantize(Decimal("0.01"))
            moderator_amount = total_amount - platform_amount
            await mod_repo.add_earning(
                moderator_id=route.creator_id,
                amount=moderator_amount,
                route_id=route_id,
                buyer_id=user.id,
                description=f"Продажа маршрута: {route.name}"
            )
            await session.execute(
                text("""
                    INSERT INTO platform_earnings
                    (route_id, buyer_user_id, moderator_id, total_amount, commission_percent, platform_amount, moderator_amount)
                    VALUES (:route_id, :buyer_id, :moderator_id, :total, :percent, :platform, :moderator)
                """),
                {
                    "route_id": route_id,
                    "buyer_id": user.id,
                    ...
                    "platform": float(platform_amount),
                    "moderator": float(moderator_amount)
                }
            )
...
async def cb_topup_for_route(
    ...
    needed_amount = int(parts[3])
    await state.update_data(pending_route_id=route_id)
    text = i18n.get("bank_deposit_amount_title", user.language)
    text += f"\n\n💡 <i>Рекомендуемая сумма для покупки: {needed_amount} грошей</i>"
    await callback.message.edit_text(
        text,
        reply_markup=BankKeyboards.deposit_amounts_with_recommended("yookassa", needed_amount, user.language),
        parse_mode="HTML"
    )
    await callback.answer()
=======
