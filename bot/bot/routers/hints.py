...
async def cmd_hint(message: Message, user: User, session: AsyncSession, state: FSMContext):
    ...
    if stats['used'] >= stats['max']:
        await message.answer(i18n.get("hints_exhausted", user_lang, default="Вы использовали все подсказки на этом маршруте."))
        return
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    from aiogram.types import InlineKeyboardButton
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(
        text=i18n.get("hint_level_easy", user_lang, default="💡 Лёгкая подсказка"),
        callback_data=f"hint:use:{point_id}:1"
    ))
    builder.row(InlineKeyboardButton(
        text=i18n.get("hint_level_medium", user_lang, default="🔦 Средняя подсказка"),
        callback_data=f"hint:use:{point_id}:2"
    ))
    builder.row(InlineKeyboardButton(
        text=i18n.get("hint_level_detailed", user_lang, default="🎯 Детальная подсказка"),
        callback_data=f"hint:use:{point_id}:3"
    ))
