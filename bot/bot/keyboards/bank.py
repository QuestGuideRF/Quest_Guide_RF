...
<<<<<<< HEAD
    ...
    def deposit_amounts_with_recommended(payment_method: str, recommended: int, lang: str = "ru") -> InlineKeyboardMarkup:
        ...
        builder.row(
            InlineKeyboardButton(
                text=f"⭐ {recommended} грошей (рекомендуемая)",
                callback_data=f"bank:deposit:{payment_method}:{recommended}"
            )
        )
        amounts = [100, 250, 500, 1000, 2000, 5000]
        amounts = [a for a in amounts if a != recommended]
        for i in range(0, len(amounts), 2):
            row_buttons = []
            for amount in amounts[i:i+2]:
                row_buttons.append(
        ...
        return builder.as_markup()
    @staticmethod
    def topup_for_route(needed_amount: int, route_id: int, lang: str = "ru") -> InlineKeyboardMarkup:
        from bot.utils.i18n import i18n
        builder = InlineKeyboardBuilder()
        builder.row(
            InlineKeyboardButton(
                text=i18n.get("bank_topup_needed", lang, amount=needed_amount),
                callback_data=f"bank:topup_for_route:{route_id}:{needed_amount}"
            )
        )
        builder.row(
            InlineKeyboardButton(
