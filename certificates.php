...
<<<<<<< HEAD
    ...
    <div class="card">
        <div class="card-body text-center py-5">
            <p class="text-muted"><?= $current_lang === 'en' ? 'Sign in to view your certificates.' : 'Войдите, чтобы видеть свои сертификаты.' ?></p>
...
<<<<<<< HEAD
                            <div class="d-flex gap-2">
                                ...
                                <a href="/api/generate_album.php?progress_id=<?= $quest['progress_id'] ?>"
...
<<<<<<< HEAD
                ...
                $display_path = $cert_for_display['file_path'];
                $display_exists = $cert_lang === 'ru' ? $ru_exists : $en_exists;
                $download_cert_href = $display_exists ? $display_path : (isset($certs['ru']) && $ru_exists ? $certs['ru']['file_path'] : ($certs['en']['file_path'] ?? '#'));
                ?>
                <div class="col-md-6 col-lg-4">
                    <div class="card h-100 certificate-card">
                        <div class="card-img-top position-relative certificate-image-container" style="min-height: 200px; background: #f5f5f5; display: flex; align-items: center; justify-content: center; padding: 15px;">
                            <?php if ($display_exists): ?>
                                <img src="<?= htmlspecialchars($display_path) ?>"
                                     alt="<?= $current_lang === 'en' ? 'Certificate' : 'Сертификат' ?>"
                                     class="w-100"
                                     style="object-fit: contain; max-height: 400px; height: auto;"
                                     onerror="this.style.display='none'; this.parentElement.innerHTML='<div class=\'d-flex align-items-center justify-content-center text-muted p-4\'>📜 <?= $current_lang === 'en' ? 'Certificate' : 'Сертификат' ?></div>'">
                            <?php else: ?>
                        ...
                        <div class="card-footer bg-transparent border-0 pt-0">
                            <div class="d-flex gap-2 flex-wrap">
                                <a href="<?= htmlspecialchars($download_cert_href) ?>"
                                   class="btn btn-primary btn-sm flex-grow-1 text-decoration-none d-inline-flex align-items-center justify-content-center gap-2"
                                   download
                                   title="<?= t('download_certificate') ?>">
                                    <i class="fas fa-download"></i>
                                    <span><?= t('download_certificate') ?></span>
                                </a>
                                <a href="/api/generate_album.php?progress_id=<?= $progress_id ?>"
                                   class="btn btn-success btn-sm flex-grow-1 text-decoration-none d-inline-flex align-items-center justify-content-center gap-2"
                                   target="_blank"
                                   title="<?= t('download_album') ?>">
