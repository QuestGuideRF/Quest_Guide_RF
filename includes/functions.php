...
function getAchievementImageUrl($achievement_id) {
    ...
    if ($id <= 0) return null;
    $path = dirname(__DIR__) . '/assets/certificate_foto/achievements/' . $id . '.png';
    return file_exists($path) ? '/assets/certificate_foto/achievements/' . $id . '.png' : null;
}
function syncUserAchievements($user_id) {
    $db = getDB();
    $earned = $db->fetchAll('SELECT achievement_id FROM user_achievements WHERE user_id = ?', [$user_id]);
    $earned_ids = array_column($earned, 'achievement_id');
    $all = $db->fetchAll('SELECT * FROM achievements ORDER BY `order`');
    $granted = 0;
    foreach ($all as $a) {
        if (in_array((int)$a['id'], $earned_ids, true)) continue;
        $ok = false;
        $type = $a['condition_type'] ?? '';
        $val = isset($a['condition_value']) ? (int)$a['condition_value'] : 1;
        switch ($type) {
            case 'routes_completed':
                $cnt = $db->fetch("SELECT COUNT(*) as c FROM user_progress WHERE user_id = ? AND status IN ('completed', 'COMPLETED')", [$user_id])['c'] ?? 0;
                $ok = $cnt >= $val;
                break;
            case 'points_completed':
                $sum = $db->fetch('SELECT COALESCE(SUM(points_completed),0) as c FROM user_progress WHERE user_id = ?', [$user_id])['c'] ?? 0;
                $ok = $sum >= $val;
                break;
            case 'photos_taken':
                $cnt = $db->fetch('SELECT COUNT(*) as c FROM user_photos WHERE user_id = ?', [$user_id])['c'] ?? 0;
                $ok = $cnt >= $val;
                break;
            case 'all_achievements':
                $cnt = $db->fetch('SELECT COUNT(*) as c FROM user_achievements WHERE user_id = ?', [$user_id])['c'] ?? 0;
                $ok = $cnt >= $val;
                break;
            case 'perfect_route':
                $rows = $db->fetchAll(
                    "SELECT up.route_id, up.points_completed FROM user_progress up WHERE up.user_id = ? AND up.status IN ('completed', 'COMPLETED')",
                    [$user_id]
                );
                foreach ($rows as $r) {
                    $total = $db->fetch('SELECT COUNT(*) as c FROM points WHERE route_id = ?', [$r['route_id']])['c'] ?? 0;
                    if ($total > 0 && (int)$r['points_completed'] >= (int)$total) { $ok = true; break; }
                }
                break;
            case 'fast_completion':
                $rows = $db->fetchAll(
                    "SELECT up.started_at, up.completed_at, r.duration_minutes FROM user_progress up JOIN routes r ON r.id = up.route_id WHERE up.user_id = ? AND up.status IN ('completed', 'COMPLETED') AND up.completed_at IS NOT NULL",
                    [$user_id]
                );
                foreach ($rows as $r) {
                    $mins = $r['duration_minutes'] ?? null;
                    if ($mins !== null && $r['started_at'] && $r['completed_at']) {
                        $diff = (strtotime($r['completed_at']) - strtotime($r['started_at'])) / 60;
                        if ($diff < (float)$mins) { $ok = true; break; }
                    }
                }
                break;
            case 'night_quest':
                $rows = $db->fetchAll("SELECT started_at FROM user_progress WHERE user_id = ? AND status IN ('completed', 'COMPLETED')", [$user_id]);
                foreach ($rows as $r) {
                    if ($r['started_at']) {
                        $h = (int)date('G', strtotime($r['started_at']));
                        if ($h >= 22 || $h < 6) { $ok = true; break; }
                    }
                }
                break;
            case 'early_bird':
                $rows = $db->fetchAll('SELECT started_at FROM user_progress WHERE user_id = ?', [$user_id]);
                foreach ($rows as $r) {
                    if ($r['started_at'] && (int)date('G', strtotime($r['started_at'])) < 8) { $ok = true; break; }
                }
                break;
            default:
                break;
        ...
        if ($ok) {
            $db->query('INSERT INTO user_achievements (user_id, achievement_id, earned_at) VALUES (?, ?, NOW())', [$user_id, $a['id']]);
            $earned_ids[] = (int)$a['id'];
            $granted++;
        }
    }
